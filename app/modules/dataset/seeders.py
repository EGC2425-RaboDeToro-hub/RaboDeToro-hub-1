    ...
    def count_features_in_uvl(self, file_path):
        ...
        with open(file_path, "r") as file:
            ...
            features_section = re.search(
                r"features\s+(.+?)constraints", content, re.DOTALL
            )
            ...
            features = re.findall(
                r'^[ \t]*"?([a-zA-Z0-9_ \-]+)"?[ \t]*$', features_content, re.MULTILINE
            )
            filtered_features = [
                f.strip()
                for f in features
                if not re.match(
                    r"^\s*(mandatory|optional|alternative|or)\s*$", f.strip()
                )
            ]
    ...
    def run(self):
        ...
        for dataset in seeded_datasets:
            ...
            for feature_model in [
                fm for fm in seeded_feature_models if fm.data_set_id == dataset.id
            ]:
